...
CONFIG_PATH = REPO_ROOT / "configs" / "cascade_convnext_eccv.py"
# Slimmed BDD100K detector (state_dict only). Bundled into the image — runtime is network-isolated.
BUNDLED_WEIGHTS = REPO_ROOT / "weights" / "cascade_convnext_bdd_slim.pth"
...
def main() -> None:
    ...
    logger = HafniaLogger(project_name="eccv-cross-city-cascade-convnext")
    logger.log_configuration(
        {
            "model": "CascadeRCNN+ConvNeXt-Tiny (mmdet3)",
